...
% constant rotation of R_{z, pi/4}
R = [cos(pi/4), -sin(pi/4), 0;
     sin(pi/4), cos(pi/4),  0;
     0,          0,           1];
% initialize empty joint space matrix
q = zeros(num_steps, 6);

for i = 1:num_steps
    % calculate desired pose at timestep i
    H = eye(4);
    H(1:3, 1:3) = R;
    H(1:3, 4) = o(i, :);
    % get robot configuration at timestep i from inverse kinematics
